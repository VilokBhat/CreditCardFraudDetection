...
def plot_and_log_roc_curve(model, X_test, y_test, model_name):
    ...
    # Save the plot
    plt_path = f"models/{model_name}_roc_curve7000.png"
...
def train_and_evaluate_model(model_name, model, X_train, y_train, X_test, y_test, cv_method):
    ...
    with mlflow.start_run(run_name=model_name):
        ...
        mlflow.sklearn.log_model(model, f"{model_name}_model7000")
        ...
        joblib.dump(model, f"models/{model_name}_model7000.pkl")
        print(f"{model_name} model saved to 'models/{model_name}_model7000.pkl' with avg accuracy: {avg_score:.4f}")

def main(cv_type='KFold', n_splits=5):
    # Load and preprocess the data
    data = load_and_preprocess('data/raw/balanced_creditcard_7000.csv')
